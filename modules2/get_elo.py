...
def __change_format_if_duplicate(team_object):  
    best_team_name = team_object.name
    print(f'best_team_name: {best_team_name}')
    if '+' in best_team_name:
        ...
        print(f'new_best_team_name: {new_best_team_name}')
...
def __format_teamname(player, team_object):  
  best_team_name = team_object.name
  if '+' in best_team_name:
      name_plus_index = best_team_name.find(' + ') # gets number of + char
      name_length = len(best_team_name)
      name_1 = best_team_name[0:name_plus_index]
      name_2 = best_team_name[name_plus_index+3:name_length]
...
def __order_team_name(player, brawl_id_one, brawl_id_two, team_obj):
  if player["brawlhalla_id"] == brawl_id_one:
    return team_obj
  else:
    return __change_format_if_duplicate(team_obj)
...
def __find_best_team(clan, player):
    ...
    team_obj = __order_team_name(player, brawl_id_one, brawl_id_two, team_obj)
