...
from modules2.embed import send_embeds
...
async def send_all_legends_elo(brawlhalla_id, bot):
  ...
  embed_title, embed_array = prep_embeds(ranked_stats, sorted_legends_ranked_stats)
  await send_embeds(embed_title, embed_array, bot, Clan('', 0, 0, [], 0x000000, '', 1165233663923994666), 1165233774305493012)
...
def prep_embeds(ranked_stats, sorted_legends_ranked_stats):
  ...
  embed_title = discord.Embed(title=name + ' Legends ELO', description='')
  # Variables
  embed_array = []
  global rank
  rank = 1
