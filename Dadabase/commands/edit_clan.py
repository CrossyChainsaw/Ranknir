from Dadabase.modules.data_management import read_data, CLANS_DATA_PATH, write_data
from Dadabase.modules.format import bool_to_show_hide, format_color


async def edit_clan(interaction, channel_1v1_id:str, channel_2v2_id:str, color:str, image, sorting_method:str, show_member_count:bool, show_xp:bool, show_no_elo_players:bool, channel_rotating_id:str, has_account_linkers:bool):
    # load data
    clan_data = read_data(CLANS_DATA_PATH, interaction.guild.id)
    print(has_account_linkers)
    ...
    if any([channel_1v1_id is not None, channel_2v2_id is not None, color is not None, image is not None,
        ...
        show_no_elo_players is not None, channel_rotating_id is not None, has_account_linkers is not None]):
        ...
        if show_member_count:
            clan_data['member_count'] = bool_to_show_hide(show_member_count)
        if show_xp:
            clan_data['xp'] = bool_to_show_hide(show_xp)
        if show_no_elo_players:
            clan_data['no_elo_players'] = bool_to_show_hide(show_no_elo_players)
        if channel_rotating_id:
            clan_data['channel_rotating_id'] = int(channel_rotating_id)
        if has_account_linkers is not None:
            clan_data['has_account_linkers'] = has_account_linkers
