...
from data.player_json_data_for_testing import json_data_void, json_data_mitchel
...
async def get_players_elo_2v2(clan, players):
    ...
    for i, player in enumerate(players):
        # player_ranked_stats = await fetch_player_ranked_stats(player['brawlhalla_id'])
        player_ranked_stats = __get_player(player['brawlhalla_id'])
        team_object = __extract_player_stats_into_team_object_2v2(
            clan, player_ranked_stats, team_object_array)
...
def __get_player(id):
    if id == 6804303:
        return json_data_void
    elif id == 10094626:
        return json_data_mitchel
...
def __extract_player_stats_into_team_object_2v2(clan, player, team_object_array):
    team_object = __find_best_team(clan, player)
    team_object.name = __format_teamname(team_object)
    team_object.name = __change_format_if_duplicate(
        team_object, team_object_array)
    team_object.name = __give_empty_name_a_placeholder_name(team_object.name)
    team_object.name = __try_decode(team_object.name)
    return team_object


def __change_format_if_duplicate(team_object, team_object_array):
    for t in team_object_array:
        if team_object.name == t.name:
            best_team = team_object.name
            if '+' in best_team:
                name_plus = best_team.find('** + **')
                print(name_plus)
                team_name_length = len(best_team)
                name_1 = best_team[0:name_plus]
                name_2 = best_team[name_plus+7:team_name_length]
                new_name = name_2 + '** + **' + name_1
                return new_name
            else:
                return best_team
...
def __format_teamname(team_object):
    best_team = team_object.name
    if '+' in best_team:
        name_plus = best_team.find('+')
